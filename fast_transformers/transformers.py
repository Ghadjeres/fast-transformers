...
class TransformerDecoderLayer(Module):
    ...
    def forward(self,
        ...
        # Secondly apply the cross attention and add it to the previous output
        # FORCED Hack so that no extra computation is made when
        # using DiagonalCrossAttention
...
class TransformerDecoderLayerWithStates(Module):
    """The decoder layer from "Attention Is All You Need".
    ...
    def __init__(self,
                 ...
                 activation="relu",
                 event_dispatcher=""):
        super(TransformerDecoderLayerWithStates, self).__init__()
        d_ff = d_ff or 4 * d_model
        self.self_attention = self_attention
        self.cross_attention = cross_attention
        self.linear1 = Linear(d_model, d_ff)
        self.linear2 = Linear(d_ff, d_model)
        self.norm1 = LayerNorm(d_model)
        if not ((self.cross_attention.__class__.__name__
                 == 'EmptyAttentionLayer') or
                (self.cross_attention.__class__.__name__
                 == 'EmptyAttentionLayerWithStates')):
            self.norm2 = LayerNorm(d_model)
        self.norm2bis = LayerNorm(d_model)
        self.norm3 = LayerNorm(d_model)
        self.dropout = Dropout(dropout)
    ...
    def forward(self,
        ...
        x2, self_state = self.self_attention(y,
                                             y,
                                             y,
                                             attn_mask=x_mask,
                                             query_lengths=x_length_mask,
        ...
        memory = self.norm2bis(memory)
        x2, cross_state = self.cross_attention(y,
                                               memory,
                                               memory,
                                               attn_mask=memory_mask,
                                               query_lengths=x_length_mask,
                                               key_lengths=memory_length_mask)
        x = self.gating2(x, self.dropout(x2))
...
class TransformerDecoderWithStates(Module):
    """TransformerDecoder is little more than a sequence of transformer decoder
    layers.
