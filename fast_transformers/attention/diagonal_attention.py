...
class DiagonalAttentionWithStates(DiagonalAttention):
    """Simply returns values and states
    ...
    """
    def __init__(self, feature_map=None, eps=1e-6, event_dispatcher=""):
        super(DiagonalAttentionWithStates, self).__init__()
        self.feature_map = feature_map or elu_feature_map
        self.eps = eps
        self.event_dispatcher = EventDispatcher.get(event_dispatcher)
...
AttentionRegistry.register(
    "diagonal-states", DiagonalAttentionWithStates,
    [
        ("feature_map", Optional(Callable)),
        ("event_dispatcher", Optional(EventDispatcherInstance, ""))
    ]
